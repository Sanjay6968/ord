...
const DispatchOrderDetailsDialog: React.FC<DispatchDetailsDialogProps> = ({
  open,
  onClose,
  orderId,
  updateOrderStatus,
  ...
  const handleStatusToggle = (event: SelectChangeEvent<string>) => {
  ...
  const handleUpdateTrackingDetails = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_MEKUVA_BACKEND_API_BASE_URL}/api/private/orders/updateTrackingDetails`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
...
export default DispatchOrderDetailsDialog;
