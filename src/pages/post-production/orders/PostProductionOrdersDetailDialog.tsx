...
interface PostProductionOrderDetailsDialogProps {
  open: boolean;
  onClose: () => void;
  orderId: string;
...
const PostProductionOrderDetailsDialog: React.FC<PostProductionOrderDetailsDialogProps> = ({
  open,
  onClose,
  orderId,
  updateOrderStatus,
  ...
  const handleStatusToggle = (event: React.ChangeEvent<HTMLInputElement>) => {
    const newStatus = event.target.checked ? 'Post Processing' : 'In Production';
    setStatus(newStatus);
                  ...
                  <CardContent>
                    <FormControl fullWidth>
                      <FormControlLabel
                        control={
                          <Switch
                            checked={status === 'Post Processing'}
...
export default PostProductionOrderDetailsDialog;
